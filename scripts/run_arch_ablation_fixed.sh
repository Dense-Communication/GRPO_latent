...
#SBATCH --job-name=arch_abl
#SBATCH --output=logs/arch_ablation_%j.out
#SBATCH --error=logs/arch_ablation_%j.err

# Architecture Ablation: Compare num_layers = 1, 2, 4 on GSM8K

source ~/.bashrc
conda activate latentmas
cd /p/scratch/westai0052/liu52/LatentMAS
mkdir -p logs/architecture checkpoints/architecture
...
LAYER_CONFIGS = [1, 4]  # 2 is our default (already in V4 results)
TASK = "gsm8k"
TOP_K = 40
TRAIN_SAMPLES = 500
TEST_SAMPLES = 500
...
print(f"Testing num_layers: {LAYER_CONFIGS}")
...
os.makedirs('checkpoints/architecture', exist_ok=True)

for num_layers in LAYER_CONFIGS:
    print('\n' + '='*60)
    print(f'TRAINING: num_layers={num_layers}')
    print('='*60)
    sys.stdout.flush()

    # Initialize policy with specific num_layers
    ...
    checkpoint_path = f'checkpoints/architecture/gsm8k_layers{num_layers}_policy.pt'
    best_acc = 0.0
    ...
    print(f'\nTraining done for layers={num_layers}, best_acc={best_acc:.2%}')

    # Evaluate
    print('\n' + '='*60)
    print(f'EVALUATING: num_layers={num_layers}')
    print('='*60)
    ...
    result_file = f'logs/architecture/gsm8k_layers{num_layers}.json'
