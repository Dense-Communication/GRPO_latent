...
#SBATCH --job-name=debug_emb
#SBATCH --output=logs/debug_emb_%j.out
#SBATCH --error=logs/debug_emb_%j.err

# Debug embedding to compare baseline vs policy
...
from methods.latent_mas import LatentMASMethod  # Original method
...
# Load a few samples
data = list(load_gsm8k(split='train'))[:1]

# Test ORIGINAL LatentMAS
print('\n' + '='*60)
print('ORIGINAL LatentMASMethod')
print('='*60)
sys.stdout.flush()

original_method = LatentMASMethod(
    model, latent_steps=args.latent_steps, temperature=0.6, top_p=0.95,
    args=args
)
original_method.task = 'gsm8k'
...
for i, item in enumerate(data):
    ...
    result = original_method.run_batch_vllm([item])[0]
    print(f"\nRaw prediction (first 300 chars):")
    raw = result.get('raw_prediction', 'N/A')
    print(f"'{raw[:300]}'" if raw else "'EMPTY'")
    print(f"\nExtracted prediction: {result.get('prediction')}")
    print(f"Correct: {result.get('correct')}")
    sys.stdout.flush()

# Test LatentMASMethodRL (Baseline)
print('\n' + '='*60)
print('LatentMASMethodRL (Baseline - no policy)')
print('='*60)
sys.stdout.flush()

rl_method = LatentMASMethodRL(
    model, latent_steps=args.latent_steps, temperature=0.6, top_p=0.95,
    args=args, reading_policy=None, top_k_blocks=args.top_k_blocks, rl_training=False,
)
rl_method.task = 'gsm8k'
...
for i, item in enumerate(data):
    ...
    result = rl_method.run_batch_vllm([item])[0]
    print(f"\nRaw prediction (first 300 chars):")
    raw = result.get('raw_prediction', 'N/A')
    print(f"'{raw[:300]}'" if raw else "'EMPTY'")
    print(f"\nExtracted prediction: {result.get('prediction')}")
