#!/usr/bin/env python3
"""Generate evaluation scripts using TRAINED policy on TEST split (to verify no overfitting)."""
...
#SBATCH --mem={mem}G
#SBATCH --time={time}
#SBATCH --job-name=test_{job_name}
#SBATCH --output=logs/test_{job_name}_%j.out
#SBATCH --error=logs/test_{job_name}_%j.err

# Evaluate TRAINED policy on TEST split: {model_name} on {task}
...
# Model configurations with their trained policy checkpoints
MODELS = {
    'qwen3_4b': {
        'name': 'Qwen3-4B',
        ...
        'checkpoints': {
            'gsm8k': '/p/scratch/westai0052/liu52/LatentMAS/checkpoints/rl_policy/qwen3_4b_gsm8k_14508789/policy_best.pt',
            'arc_challenge': '/p/scratch/westai0052/liu52/LatentMAS/checkpoints/rl_policy/qwen3_4b_arc_challenge_14508790/policy_best.pt',
            'winogrande': '/p/scratch/westai0052/liu52/LatentMAS/checkpoints/rl_policy/qwen3_4b_winogrande_14508791/policy_best.pt',
        },
    },
    'qwen3_8b': {
        'name': 'Qwen3-8B',
        ...
        'checkpoints': {
            'gsm8k': '/p/scratch/westai0052/liu52/LatentMAS/checkpoints/rl_policy/qwen3_8b_gsm8k_14508792/policy_best.pt',
            'arc_challenge': '/p/scratch/westai0052/liu52/LatentMAS/checkpoints/rl_policy/qwen3_8b_arc_challenge_14508793/policy_best.pt',
            'winogrande': '/p/scratch/westai0052/liu52/LatentMAS/checkpoints/rl_policy/qwen3_8b_winogrande_14508794/policy_best.pt',
        },
    },
    'qwen3_14b': {
        'name': 'Qwen3-14B',
        ...
        'checkpoints': {
            'gsm8k': '/p/scratch/westai0052/liu52/LatentMAS/checkpoints/rl_policy/qwen3_14b_gsm8k_14508795/policy_best.pt',
            'arc_challenge': '/p/scratch/westai0052/liu52/LatentMAS/checkpoints/rl_policy/qwen3_14b_arc_challenge_14508796/policy_best.pt',
            'winogrande': '/p/scratch/westai0052/liu52/LatentMAS/checkpoints/rl_policy/qwen3_14b_winogrande_14508797/policy_best.pt',
        },
    },
}

# Dataset configurations - using TEST splits
DATASETS = {
    'gsm8k': {
        'load_fn': 'load_gsm8k',
        'split': 'test',  # 1319 samples, unseen during training
        'n_samples': 500,
        'time': '03:00:00',
    },
    'arc_challenge': {
        'load_fn': 'load_arc_challenge',
        'split': 'test',  # 1172 samples, unseen during training
        'n_samples': 500,
        'time': '03:00:00',
    },
    'winogrande': {
        'load_fn': 'load_winogrande',
        'split': 'validation',  # 1267 samples, unseen during training (no test split)
        'n_samples': 500,
        'time': '03:00:00',
    },
}
...
def main():
    ...
    for model_key, model_cfg in MODELS.items():
        for task_key, task_cfg in DATASETS.items():
            ...
            checkpoint = model_cfg['checkpoints'].get(task_key)
            if not checkpoint or not os.path.exists(checkpoint):
                print(f'Skipping {model_key}_{task_key}: checkpoint not found at {checkpoint}')
                continue
            ...
            script_path = os.path.join(scripts_dir, f'test_{job_name}.sh')
    ...
    print(f'\nTotal scripts generated: {generated}')
